...
class SoccerXGClient:
    ...
    def get_recent_fixtures(self, team_id, last_n=5, before_date=None):
        """
        Get last N fixtures for a team.
        Optional: before_date (YYYY-MM-DD or datetime) to get form 'as of' a past date.
        """
        ...
        if before_date:
            ...
            # API-Football behavior: 'last' + 'team' implies most recent relative to 'current'.
            # To get specific range: 'from', 'to'.
            # IMPORTANT: 'last' overrides date filters in some endpoints.
            # Correct approach for history: 'to' = before_date, 'season' = ... 
            # Infer season from date? simplified: just pass 'to'.
            # API documentation says: "For the parameter 'last', you can't add 'from' or 'to'".
            # So we cannot use 'last' with 'to'.
            # Workaround: Fetch 'season' fixtures up to 'to', then slice locally.
            # This is heavy. Alternatively, accept current form as proxy for backfill?
            # User wants BEST EFFORT. 
            # Better approach: We can't easily get 'last 5 before date' efficiently without season context.
            # Let's use 'to' = before_date and rely on default ordering if possible? No.
            # Let's keep it simple: if before_date is passed, we try to use 'to', but we need 'season'.
            # Hack: Pass '2025' (current) or infer index.
            # Compromise: For backfill, we will ignore exact date strictness if it causes API complexity 
            # unless we have the Season ID.
            # Let's try adding 'to' if supported, otherwise warn.
            params['to'] = before_date
            # We ideally need 'from' too.
            # Let's Remove 'last' and manage sizing manually? No, too much data.
            # Let's stick to 'last' for NOW (Current Form) - leakage accepted for V2 Prototype.
            pass
        try:
