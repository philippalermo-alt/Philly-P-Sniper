...
def get_db():
    ...
    try: return psycopg2.connect(Config.DATABASE_URL, sslmode='require')
    ...
    for sport, suffix in endpoints.items():
        ...
        try:
            ...
            for g in games:
                ...
                home_name = next((t.get('full_name') for t in teams if t.get('is_home') is True), None)
                away_name = next((t.get('full_name') for t in teams if t.get('is_home') is False), None)
